...
export const GlareCard = ({
  ...
  return (
    <div
      ...
      className="duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] relative isolate w-[250px] transition-transform will-change-transform [aspect-ratio:17/21] [contain:layout_style] [perspective:600px] xs:w-[310px]"
      ref={refElement}
      onPointerMove={(event) => {
    ...
    >
      <div className="duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] backface-visibility-hidden grid h-full origin-center overflow-hidden rounded-[var(--radius)] border border-slate-800 p-1 transition-transform will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]">
        <div className="grid h-full w-full mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] [grid-area:1/1]">
          <div
            className={cn(
              'h-full w-full border border-[#334155] bg-gradient-to-bl from-[#1E293B] to-[#0F172A] antialiased',
              className
            )}
          >
            {children}
